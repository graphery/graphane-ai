...
import assistant         from './graphane-assistant.js';
import file              from './graphane-assistant-file.js';
import improve           from './graphane-assistant-improve.js';
...
const endPoints = [
    '/assistants/will/',
  '/assistants/humor/',
  '/assistants/graphane-assistant/',
  '/assistants/graphane-assistant-file/',
  '/assistants/graphane-assistant-improve/',
...
const routes = [
  ...
  { // Run the assistant
    method  : 'POST',
    url     : `/assistants/graphane-assistant/`,
    handler : async (request, reply) => {
      try {
  ...
  { // Run the assistant
    method  : 'POST',
    url     : `/assistants/graphane-assistant-improve/`,
    handler : async (request, reply) => {
      try {
        const result = await improve(request.body);
        if (result) {
          reply.send({ok : true, result});
        } else {
          reply.code(400).send({ok : false});
  ...
  { // Run the assistant
    method  : 'POST',
    url     : `/assistants/graphane-assistant-file/`,
    handler : async (request, reply) => {
      try {
        const result = await file(request.body);
        if (result) {
          reply.send({ok : true, result});
        } else {
          reply.code(400).send({ok : false});
